...
module set_cache (
    ...
    */
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (curr_clr_addr == SETS - 1) state <= CACHE_IDLE;
    ...
    always_comb begin
        ...
        for (uint i = 0; i < WAYS; i++) begin
            ...
            cmp_in_valid[i] = cmp_in_meta[i].valid;
        end
        ...
        */
        for (uint i = 0; i < WAYS; i++) begin
            ...
            hit_line |= ( cmp_in_line[i] & {$bits(line_t){cmp_out[i]}} );
        end
    ...
    */
    ...
    always_comb begin
        ...
        bus.busy  = (state != CACHE_IDLE);  // consider OR-ing with rst...
